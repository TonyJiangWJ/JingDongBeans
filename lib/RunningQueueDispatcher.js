...
let lockableStorages = require('./LockableStorage.js')(runtime, this)

let pwd = files.cwd()
let logLibExists = files.exists(pwd + '/lib/LogUtils.js')
let _logUtils = typeof LogUtils === 'undefined' && logLibExists ? require(pwd + '/lib/LogUtils.js') : {
  logInfo: (str) => {
    console.log(str)
  },
  warnInfo: (str) => {
...
if (logLibExists) {
  console.verbose('日志工具类存在')
} else {
  console.verbose('日志工具类不存在, 当前目录：' + pwd)
}
    ...
    if (storedLockStr) {
      ...
      if (storedLock.source === taskInfo.source) {
        ...
        if (parseInt(storedLock.timeout) < new Date().getTime()) {
          ...
          return this.getStorage().put(WRITE_LOCK_KEY, JSON.stringify({ source: taskInfo.source, count: 1, timeout: new Date().getTime() + 30000 }))
        }
        return false
      }
    } else {
